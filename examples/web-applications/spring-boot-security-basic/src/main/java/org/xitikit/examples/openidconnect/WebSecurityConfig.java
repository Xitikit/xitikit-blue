...
public class WebSecurityConfig{
    ...
    public static class SpringSecurityConfig extends AbstractWebSecurityConfigurerAdapter{
        ...
        protected void configure(HttpSecurity http) throws Exception{
            ...
            http.portMapper().http(httpPort).mapsTo(httpsPort).and()
                .csrf().disable().cors().and()
                .authorizeRequests().antMatchers(HttpMethod.OPTIONS).permitAll().and()
                .authorizeRequests().antMatchers("/errors/**").anonymous().and()
                .authorizeRequests().antMatchers("/policy-templates/**").anonymous();
            if(httpsEnabled){
