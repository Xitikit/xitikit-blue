app.controller('TestingController', ['$scope', '$log', 'usSpinnerService', 'TestingService', function($scope, $log, usSpinnerService, TestingService){
  ...
  $scope.onSearch = function(){
    ...
    TestingService.performSearch($scope.searchInput)
      .then(performSearchSuccess, performSearchFailure);
  ...
  $scope.onReassign = function(){
    if($scope.selectedSearchResult){
      ...
      TestingService.reassignClient($scope.currentWebAccountId, selectedPerson)
        .then(onReassignSuccess, onReassignFailure);
      usSpinnerService.spin('loading-spinner');
  ...
  $scope.onClientLookup = function(){
    ...
    usSpinnerService.spin('loading-spinner');
    TestingService.lookupClient($scope.emailAddress)
      .then(clientLookupSuccess, clientLookupFailure);
  ...
  $scope.onClientDelete = function(){
    usSpinnerService.spin('loading-spinner');
    TestingService.deleteClient($scope.currentWebAccountId)
      .then(clientDeleteSuccess, clientDeleteFailure);
  ...
  $scope.onClientDisconnect = function(){
    usSpinnerService.spin('loading-spinner');
    TestingService.disconnectClient($scope.currentWebAccountId)
      .then(clientDisconnectSuccess, clientDisconnectFailure);
